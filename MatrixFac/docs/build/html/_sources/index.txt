...
Seeking to find the most relevant items on the internet with little efforts has became increasingly popular in the last years. Recommender systems are bringing a contribution to this issue by using algorithms that will analyze the user data and give recommendations based on the results.
...
^^^^^^^^^^^^^^^^^^^^^^
The programming language used for the implementation of algorithm is Python, version 3.5.
